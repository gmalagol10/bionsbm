...
for latent_dim in [2,6,12,18,24]:
	for run in range(0, 5):
		...
		print(time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime()), f"Latent dim {latent_dim} Run {run}/5 Fitting Mowgli model", flush=True)
		model.train(mdata, device=device)

		print(time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime()), f"Latent dim {latent_dim} Run {run}/5 Saving results", flush=True)
		...
		embeddding.to_csv(f"{folder}/LatentDim/LT_{latent_dim}/Run{run}/{name}_Embedding.tsv.gz", compression="gzip", sep="\t")
