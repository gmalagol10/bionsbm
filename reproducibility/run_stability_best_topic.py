...
features = sc.read_h5ad(f"Datasets/{dataset}/CM/{dataset}_{main_fs}_Def.h5ad").var.index
...
for run in range(n_runs):
    ...
    for fs in order:
        ...
        for fname in os.listdir(base_dir):
            if not fname.endswith(f"_{main_fs}_topics.tsv.gz"):
                continue
        ...
        td_file = best_file.replace(f"_{main_fs}_topics.tsv.gz",f"_{main_fs}_topics_documents.tsv.gz")
