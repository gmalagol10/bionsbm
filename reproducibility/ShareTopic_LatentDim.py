...
for latent_dim in [2,6,12,18,24]:
	for run in range(0, 5):
		...
		alpha=50/latent_dim
		print(time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime()), f"Latent dim {latent_dim} Starting Share-Topic run {run}", flush=True)
		st_obj = SHARE_topic(mdata[key1], mdata[key2], latent_dim, alpha, beta, gamma, tau)
		...
		print(time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime()), f"Latent dim {latent_dim} Saving results run {run}", flush=True)
		...
		top_cell=pd.DataFrame(m_theta.cpu().detach().numpy(), index=mdata[key2].obs.index, columns=[f"Topic_{t}" for t in range(0, latent_dim)]).T
		top_key1=pd.DataFrame(m_phi.cpu().detach().numpy(), index=[f"Topic_{t}" for t in range(0, latent_dim)], columns=mdata[key1].var.index)
		top_key2=pd.DataFrame(m_lam.cpu().detach().numpy(), index=[f"Topic_{t}" for t in range(0, latent_dim)], columns=mdata[key2].var.index)

		d=pd.DataFrame(list(np.argmax(top_cell.values, axis=0).astype(str)), columns=[f"Run{run}"], index=mdata[key1].obs.index)
		top_cell.to_csv(f"{folder}/LatentDim/LT_{latent_dim}/Run{run}/{name}_Topic_Cell.tsv.gz", compression="gzip", sep="\t")
		top_key1.to_csv(f"{folder}/LatentDim/LT_{latent_dim}/Run{run}/{name}_Topic_{key1}.tsv.gz", compression="gzip", sep="\t")
		top_key2.to_csv(f"{folder}/LatentDim/LT_{latent_dim}/Run{run}/{name}_Topic_{key2}.tsv.gz", compression="gzip", sep="\t")
		d.to_csv(f"{folder}/LatentDim/LT_{latent_dim}/Run{run}/{name}_MaxTopic.tsv.gz", compression="gzip", sep="\t")
